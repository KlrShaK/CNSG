...
MAX_USER_INPUT_ITERATIONS = 1000


TEST_PHRASES = [
    # Rooms
    "How do I get to the upper bedroom?",
    "Where is the upper bathroom?",
    "Can you guide me to the office?",
    "How do I reach the living room?",
    "Where is the kitchen located?",
    "I need to find lower bathroom 1.",
    "How can I get to the entryway?",
    "Where is the dining room?",
    "How do I go to the lower bedroom?",
    "Where is lower bathroom 2?",
    "Can you show me the way to the dressing room?",
    "Where can I find a bathroom?",
    "How do I get to the bedroom?",

    # Objects
    "Where is the chandelier?",
    "Can you help me find the tv?",
    "Where is the bed located?",
    "I need to find the plush toy.",
    "Where can I find an armchair?",
    "Where is the toilet?",
    "How do I reach the bicycle?",
    "Where is the couch?",
    "Can you help me find the computer desk?",
    "Where is the printer?",
    "I want to locate the plant.",
    # "How do I get to the stairs?",
    "Where is the fireplace?",
    "Where can I find the flag?",
    "How do I reach the refrigerator?",
    "Where is the kitchen cabinet?",
    "Where can I find the coffee machine?",
    "I need to locate the microwave.",
    "Where is the oven and stove?",
    "Where do I find the dishwasher?",
    "Where is the bath sink?",
    "Where are the clothes stored?",
    "Where can I find the doormat?",

    # Combined: object + room
    "Where is the tv in the upper bedroom?",
    "How do I reach the plush toy in the office?",
    "Where is the plush toy in the upper bedroom?",
    "Where can I find the chandelier in the dining room?",
    "How do I get to the bed in the bedroom?",
    "Where is the armchair in the living room?",
    "Where is the couch in the living room?",
    "How do I reach the computer desk in the office?",
    "Where is the printer in the office?",
    "Where can I find the plant in the office?",
    "How do I reach the fireplace in the living room?",
    "Where is the refrigerator in the kitchen?",
    "How do I find the kitchen cabinet in the kitchen?",
    "Where is the coffee machine in the kitchen?",
    "Where can I find the microwave in the kitchen?",
    "How do I get to the oven and stove in the kitchen?",
    "Where is the dishwasher in the kitchen?",
    "Where is the bath sink in the bathroom?",
    "How do I find the clothes in the dressing room?",
    "Where is the doormat in the entryway?",
]
...
class NewViewer(BaseViewer):
    ...
    def start_local_llm_navigation(self, user_input, model, tokenizer, output_q):
        ...
        # randomize the position of the agent on the navigable mesh
        viewer.randomize_agent_on_navmesh()
        
        self.start_navigation(sim=self.sim, candidate_goals=checked_candidate_goals, user_input=user_input, output_q=output_q)


    def randomize_agent_on_navmesh(self, use_seed=True):
        if not self.sim.pathfinder.is_loaded:
            print("Pathfinder not initialized, cannot randomize agent position.")
            return

        # Set a different seed each time for true randomization
        if use_seed:
            # Use current time + random component for unique seed
            import time
            random_seed = int(time.time() * 1000) % 100000 + np.random.randint(0, 10000)
            self.sim.pathfinder.seed(random_seed)
            np.random.seed(random_seed)
            print(f"Using random seed: {random_seed}")

        # Randomly sample a point on the navmesh
        random_point = self.sim.pathfinder.get_random_navigable_point()
        if random_point is None:
            print("No navigable points found.")
            return

        # Set the agent's position to the random point
        agent_state = self.sim.get_agent(self.agent_id).get_state()
        agent_state.position = random_point
        
        # Randomize agent rotation
        random_yaw = np.random.uniform(0, 2 * np.pi)
        agent_state.rotation = quat_from_angle_axis(random_yaw, np.array([0, 1, 0]))
        self.sim.get_agent(self.agent_id).set_state(agent_state)

        print(f"Agent randomized to position: {random_point}, rotation: {random_yaw:.2f} rad")
...
def user_input_logic_loop(viewer: NewViewer, input_q: queue.Queue, output_q: queue.Queue, model=None, tokenizer=None):
    ...
    while True:
        ...
        # count the number of lines in file "data_collection.jsonl"
        with open("data_collection.jsonl", "r") as f:
            line_count = sum(1 for line in f)
        if line_count >= MAX_USER_INPUT_ITERATIONS:
            print(f"Reached maximum number of user input iterations: {MAX_USER_INPUT_ITERATIONS}. Exiting loop.")
            # exit the program
            os._exit(0)
        # get a index from 0 to len(TEST_PHRASES)-1
        phrase_idx = i % len(TEST_PHRASES)
        i += 1
        try:
            user_input = TEST_PHRASES[phrase_idx]
